            ...
            <div class="form-group">
                ...
                <div class="upload-area" id="upload-area">
                    ...
                    <div class="upload-text">
                        Arraste sua foto ou vídeo aqui<br>
...
<script>
    ...
    async function handleFileSelect(input) {
        ...
        uploadStatus.textContent = isVideo ? 'Enviando vídeo... pode demorar' : 'Enviando imagem...';
    ...
    async function handlePost() {
        if (!uploadedUrl) return showAlert('Faça o upload primeiro!', 'error');
        ...
        if (scheduleTime) {
            ...
            try {
                const res = await fetch('/schedule', {
                    ...
                    body: JSON.stringify({ media_url: uploadedUrl, caption, schedule_time: scheduleTime, post_type: postType, is_video: isVideo })
                });
                const data = await res.json();
            ...
            try {
                const res = await fetch('/publish-now', {
                    method: 'POST',
                    headers: {'Content-Type': 'application/json'},
                    body: JSON.stringify({ media_url: uploadedUrl, caption, post_type: postType, is_video: isVideo })
                });
                const data = await res.json();
            ...
            list.innerHTML = posts.slice().reverse().map(p => `
                <div class="post-item">
                    <div class="post-thumb">
                        ${p.is_video
                            ? '<div style="display:flex;flex-direction:column;align-items:center;gap:2px"><span>🎬</span><span style="font-size:8px;color:var(--muted)">' + p.post_type.toUpperCase() + '</span></div>'
                            : (p.media_url ? `<img src="${p.media_url}" onerror="this.parentElement.innerHTML='📷'">` : '📷')
                        }
                    </div>
                    <div class="post-info">
